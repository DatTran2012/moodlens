    ...
    if (!item) return (
        <div className="min-h-screen flex items-center justify-center" style={{ background: P.bg }}>
            <div className="animate-pulse space-y-4 w-full max-w-2xl px-4">
                ...
                <div className="h-56 rounded-2xl" style={{ background: P.card }} />
                <div className="h-28 rounded-2xl" style={{ background: P.card }} />
            </div>
        </div>
    );
            ...
            <div className="max-w-2xl mx-auto px-4 py-4 sm:px-6 sm:py-8">
                        ...
                        {editing ? (
                            ...
                            <>
                                ...
                                <button onClick={handleDelete} disabled={deleting}
                                    ...
                                    <BsTrash3 size={12} />
                                    <span className="hidden sm:inline">{deleting ? "Xoá..." : "Xoá"}</span>
                                </button>
                            </>
                        )}
                    </div>
                ...
                <div className="relative rounded-2xl overflow-hidden mb-4"
                    style={{
                        ...
                        boxShadow: "0 4px 20px rgba(139,110,80,0.10), inset 0 0 0 1px rgba(255,255,255,0.5)"
                    }}>
                    {/* Gáy sổ trái */}
                    ...
                    <div className="relative p-4 sm:p-6 pl-7 sm:pl-9">
                        ...
                        <div className="flex items-start justify-between mb-4 pb-3"
                            ...
                            {/* Mood badge — nhỏ hơn trên mobile */}
                            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full
                                            border text-xs font-semibold shrink-0 ml-2
                                            ${mood.bg} ${mood.border} ${mood.color}`}>
                                <span>{mood.emoji}</span>
                                <span className="capitalize hidden sm:inline">{item.mood}</span>
                            </div>
                        </div>
                ...
                {/* ── ĐIỂM SỐ + AI SUMMARY — 1 row trên mobile ── */}
                    ...
                    <div className="rounded-2xl p-4"
                        ...
                        <div className="relative mb-1.5">
                            <div className="flex h-2.5 rounded-full overflow-hidden"
                                ...
                                {moodRanges.map(r => (
                                    <div key={r.label} className={`${r.color} flex-1 opacity-75`} title={r.label} />
                                ))}
                            </div>
                            <div className="absolute top-[-4px] w-4 h-4 rounded-full shadow-lg transition-all duration-500"
                                style={{ left: `calc(${score}% - 8px)`, background: "white", border: `2px solid ${P.spine}` }} />
                        </div>
