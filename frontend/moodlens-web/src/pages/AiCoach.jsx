...
// ── ChatPanel — NGOÀI component để tránh re-mount mỗi lần render ─────────────
const ChatPanel = memo(({
    messages, loading, question, setQuestion,
    askAI, createConversation, onSuggestion,
    bottomRef, handleKeyDown
}) => (
    <div className="rounded-2xl flex flex-col overflow-hidden h-full"
        ...
        {/* Suggestions */}
        <div className="px-3 py-2.5 flex gap-2 overflow-x-auto shrink-0"
...
// ── RightPanel — NGOÀI component ─────────────────────────────────────────────
const RightPanel = memo(({
    conversations, conversationId, insight,
    createConversation, openConversation,
    renameConversation, deleteConversation, togglePin
}) => (
...
export default function AiCoach() {
    ...
    const loadInsight = async () => {
        try { const res = await api.get("/aicoach/coach-insight"); setInsight(res.data); }
    ...
    const typeMessage = async (text) => {
        const mid = Date.now();
        setMessages(prev => [...prev, { id:mid, role:"assistant", content:"" }]);
        let cur = "";
        for (let i = 0; i < text.length; i++) {
            cur += text[i];
            setMessages(prev => prev.map(m => m.id === mid ? { ...m, content:cur } : m));
    ...
    const askAI = async () => {
        ...
        try {
            ...
            if (!cid) {
                const res = await api.post("/aicoach/conversation");
                cid = res.data.id; setConversationId(cid); loadConversations();
            }
            const res = await api.post("/aicoach/ask", { conversationId:cid, question:q });
    ...
    const renameConversation = async (id, currentTitle) => {
        ...
        toast.success("Đổi tên thành công"); loadConversations();
    };
    ...
    const togglePin = async (id) => {
        ...
        toast.success("Đã cập nhật ghim"); loadConversations();
    };
            ...
            <div className="xl:hidden flex flex-col" style={{ height:"100dvh" }}>
                ...
                <div className="flex shrink-0 p-3 gap-2"
                    ...
                    <button onClick={() => setMobileTab("chat")}
                            ...
                            style={{ background: mobileTab==="chat" ? P.accent : P.card, color: mobileTab==="chat" ? "white" : P.muted, border:`1px solid ${P.border}` }}>
                        <BsChatDots size={15} /> Chat
                    </button>
                    <button onClick={() => setMobileTab("history")}
                            className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition"
                            style={{ background: mobileTab==="history" ? P.accent : P.card, color: mobileTab==="history" ? "white" : P.muted, border:`1px solid ${P.border}` }}>
                ...
                <div className="flex-1 min-h-0 overflow-hidden">
                    {mobileTab === "chat"
                        ? <ChatPanel {...chatProps} />
                        : <div className="h-full overflow-y-auto p-4"><RightPanel {...rightProps} /></div>
                    }
                </div>
            </div>
